...
export default function KeyContacts({ lead, onUpdateLead }: KeyContactsProps) {
  ...
  if (isLoading) {
    return (
      <div className="space-y-4">
        <h4 className="font-medium flex items-center gap-2"><UserSearch className="h-4 w-4 text-muted-foreground" /> AI Finding Key Contacts...</h4>
  ...
  if (!contacts || contacts.length === 0) {
    return (
        <div className="flex items-start gap-3 text-sm">
            <UserSearch className="h-5 w-5 flex-shrink-0 text-muted-foreground" />
            <div className="flex-grow">
                <p className="font-semibold">Key Contacts</p>
                <p className="text-muted-foreground">No verifiable key contacts were found for this company.</p>
            </div>
        </div>
    );
  }
        ...
        {contacts.map((contact, index) => (
            <div key={index} className="p-3 rounded-lg bg-secondary/30 border border-primary/10">
                ...
                <div className="mt-2 space-y-1 text-xs">
                    ...
                    {contact.twitter && (
                      <a href={contact.twitter} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 hover:underline">
                          <TwitterIcon className="h-3 w-3" /> Twitter/X Profile
                      </a>
                    )}
                </div>
            </div>
